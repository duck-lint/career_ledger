...
const probeResult = await runStep(
  'cargo-run-probe',
  cargoCommand,
  ['run', '--manifest-path', 'src-tauri/Cargo.toml', '--', '--i07-probe'],
  600_000,
);

const probeLine = probeResult.output
  .split(/\r?\n/)
  .find((line) => line.startsWith('I07_PROBE:'));

if (!probeLine) {
  console.error(JSON.stringify({
    pass: false,
    reason: 'Missing I07_PROBE line from live desktop run.',
...
const summary = JSON.parse(probeLine.slice('I07_PROBE:'.length));
const failureReasons = [];

assertEqual(summary.runtimeError, null, 'runtimeError', failureReasons);
assertEqual(summary.requirementRegionAuthority, 'sqlite', 'requirementRegionAuthority', failureReasons);
...
console.log(JSON.stringify({
  probeName: 'I07 Desktop Probe: Requirement-Region Authority Is SQLite-Only',
  pass,
  buildExitCode: buildResult.exitCode,
  probeExitCode: probeResult.exitCode,
  failureReasons,

...
| C-002 | Diagnostics model slice | `npx vitest run src/lib/taxonomy-diagnostics.test.ts src/components/taxonomy/TaxonomyDiagnosticsPanel.test.tsx` | Focused diagnostics tests pass before and after the UI interaction change. | pass | local validation on 2026-04-29 |
| C-003 | Taxonomy view interaction slice | `npx vitest run src/components/views/TaxonomyView.test.tsx` | A diagnostic item triggers the intended repair flow. | pass | local validation on 2026-04-29 |
